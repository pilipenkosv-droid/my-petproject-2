 ...
 *
 * ВАЖНО: библиография всегда в последней трети документа.
 * Если "Список литературы" встречается раньше (TOC entry, heading) — это не начало библиографии.
 */
function findBibliographyStart(
  ...
  // Библиография не может начинаться в первой половине документа
  const minStartIdx = Math.floor(totalParagraphs * 0.4);

  // Сигнал 1: AI-разметка bibliography_title (ищем ПОСЛЕДНИЙ, не первый)
  let lastBiblioTitleIdx = -1;
  for (const { paragraphIndex } of paragraphs) {
    const enriched = enrichedMap.get(paragraphIndex);
    if (enriched?.blockType === "bibliography_title" && paragraphIndex >= minStartIdx) {
      lastBiblioTitleIdx = paragraphIndex;
    }
  }
  if (lastBiblioTitleIdx >= 0) return lastBiblioTitleIdx;

  // Сигнал 2: текстовый поиск заголовка библиографии
  // Пропускаем toc/toc_entry/heading — там тоже может быть текст "Список литературы"
  for (const { paragraphIndex, node } of paragraphs) {
    if (paragraphIndex < minStartIdx) continue;
    const enriched = enrichedMap.get(paragraphIndex);
