...
| `MARKUP_BUDGET_MS` | `300000` — бюджет LLM-разметки блоков на этапе `confirm-rules`. Дефолт в коде 25 с рассчитан на 60-секундную функцию Vercel; на воркере потолок задаёт дедлайн этапа (10 мин) |
| `WORKER_ID` | `vds-1` |
| `NODE_ENV` | `production` |
...
Перед деплоем ветки: дописать `MARKUP_BUDGET_MS=300000` в
`/etc/diplox-worker.env` и перезапустить службу. Без переменной этап
`confirm-rules` отработает со старым бюджетом в 25 секунд — не сломается, но
разметка чаще будет деградировать до ключевых слов.
