...
- Регулярка запрещённого продукта в `config.toml` отличается от буквальной из
  ADR: та без границ слова ловит «раБОТа» и валит любую статью. Список слов тот
  же, добавлены границы и словоформы (`бота`, `ботами`, `телеграмм`).
