...
// against baseline bench-reports/v6/baseline-2026-04-21.json.
//
// Usage: npx tsx scripts/pipeline-v6/measure-delta.ts [count=3]
...
async function main() {
  ...
  if (results.length) {
    const avgBase = results.filter((r) => r.baselineScore !== null).reduce((a, r) => a + r.baselineScore!, 0) / results.length;
    const avgV6 = results.reduce((a, r) => a + r.v6Score, 0) / results.length;
    console.log(`\n=== Summary (${results.length} docs) ===`);
    console.log(`avg baseline: ${avgBase.toFixed(1)}`);
    console.log(`avg v6: ${avgV6.toFixed(1)}`);
    console.log(`delta: ${(avgV6 - avgBase).toFixed(1)}`);
  }
}
