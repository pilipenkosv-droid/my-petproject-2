...
Долгая обработка документов выполняется не в Vercel-функции, а воркером на
Timeweb-сервере `194.87.43.23` (тот же, где crontab и pandoc-svc). Переменные
Vercel: `PROCESSING_MODE=inline|shadow|worker` (по умолчанию `inline`),
`WORKER_PERCENT` (0–100). Установка, выкладка, откат и первый запуск описаны в
`ops/worker/DEPLOY.md`; кратко: `bash ops/worker/deploy.sh` собирает бандл на
Mac, копирует на сервер и перезапускает `diplox-worker.service`. Выкладывать
воркер нужно **до** деплоя роутов. Стоп-кран — `PROCESSING_MODE=inline`.
...
На сервере дополнительно: `*/15 * * * *` → `GET /api/health/worker` (письмо
при не-200) и `0 4 * * *` → очистка `/tmp` от каталогов `pandoc-v6-*`, `v6-*`
старше двух часов.
